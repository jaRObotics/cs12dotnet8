...
using JaroAlias = (string J1, int J2, int J3);
...
internal partial class Program
{
    ...
    {
        //Jaro test
        JaroMethod(); //it's called from Main so needs to be static 
        //JaroNonStaticmethod(); //non-static method cannot be used inside static method (Main)
        ...
        Person bob = new Person(); // C# 1 
        //var bob2 = new Person(); // C# 3 
        //Person bob = new(); // C# 9 
        ...
        */
        ...
        // bob.BucketList = (WondersOfTheAncientWorld)18; //this is possible too!
        ...
        alfred.Name = "Alfred";
        bob.Children.Add(alfred); //add person to List<Person> in Person class!!!

        alfred.Children.Add(bob); // this is possible now! 
        alfred.Children.Add(alice); // this is possible now!
        alfred.Children.Add(alfred); // this is possible now!

        // Works with C# 3 and later.
        bob.Children.Add(new Person { Name = "Bella" });

        // Works with C# 9 and later.
        bob.Children.Add(new() { Name = "Zoe" }); //this is interesting!
        ...
        */
        ...
        //the List<Person> could be created here - so it belongs to this class only 
        ...
        //Book book = new(); //this does not even compile!

        /* 
        // Instantiate a book using object initializer syntax - this works fine with "required" fields 
        Book book = new()
        {
          Isbn = "978-1803237800",
        ...
        */
        ...
        WriteLine(bob.OptionalParameters(3, "Jump!", 98.5)); //no naming - sequence must be kept

        #endregion

        #region Naming parameter values when calling methods

        WriteLine(bob.OptionalParameters(3, number: 52.7, command: "Hide!")); //different oder possible!
        ...
        bob.PassingParameters(e, f, ref g, out int h); //h declared here 
        ...
        (string, int, int) fruit = bob.GetFruit();
        JaroAlias fruit2 = bob.GetFruit(); //JaroAlias in action!
        WriteLine($"{fruit.Item1}, {fruit.Item2} there are. {fruit.Item3}");
        WriteLine($"{fruit2.Item1}, {fruit2.Item2} there are. {fruit2.Item3}"); //JaroAlias in action!
        ...
        var thing1 = ("Neville", 4); // (tuple)
        WriteLine($"{thing1.Item1} has {thing1.Item2} children.");

        var thing2 = (bob.Name, bob.Children.Count); //(tuple)
        ...
        (string fruitName, int fruitNumber, int fruitJaro) = bob.GetFruit();
        ...
        {
            decimal flightCost = passenger switch //switch modern syntax 
            {
                ///* C# 8 syntax - MORE SPREAD OUT SYNTAX 
                ///priority of steps here is kept!
                FirstClassPassenger whatever when whatever.AirMiles > 35_000 => 1_500M,
                FirstClassPassenger p when p.AirMiles > 15_000 => 1_750M,
                FirstClassPassenger _                          => 2_000M,
                //*/

                // C# 9 or later syntax - MORE COMPACT SYNTAX
                //FirstClassPassenger p => p.AirMiles switch //nested switch!!!! SHOCKING
                //{
                //    > 35_000 => 1_500M,
                //    > 15_000 => 1_750M,
                //    _ => 2_000M
