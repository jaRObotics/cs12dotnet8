...
internal partial class Program
{
    private static void Main(string[] args)
    {
        ...
        if (lamech + zillah) //operator as method in action (it executes Marry() so it's not so super-duper really 
        {
        ...
        Person baby3 = lamech * adah; //operator as method here
        baby3.Name = "Jubal";
        ...
        Array.Sort(people); //sorting? our type must contract IComparable!
        ...
        DisplacementVector dv3 = dv1 + dv2; //operator method in action 

        WriteLine($"({dv1.X}, {dv1.Y}) + ({dv2.X}, {dv2.Y}) = ({dv3.X}, {dv3.Y})");

        DisplacementVector dv4 = new(); //parameterless ctor - what values?? 
        WriteLine($"({dv4.X}, {dv4.Y})"); //gives: 0, 0 ! (because record struct is a value type - goes to stack so all its values need to be initialized

        DisplacementVector dv5 = new(3, 5);
        WriteLine($"dv1.Equals(dv5): {dv1.Equals(dv5)})");
        WriteLine($"dv1 == dv5: {dv1 == dv5})"); //ERROR for struct type!!
