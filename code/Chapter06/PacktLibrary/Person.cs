...
public class Person : IComparable<Person?> //to enable sorting 
    //IComparable resides in System
    //IComparable has only method Compare definition -> no implementation! 
{
    #region Properties

    public string? Name { get; set; }
    public DateTimeOffset Born { get; set; }
    public List<Person> Children = new(); //again, List<Person> inside class Person :)
    ...
    public bool Married => Spouses.Count > 0; //getter true if > 0, false if = 0
    ...
    {
        string term = Children.Count == 1 ? "child" : "children"; //ternary operator 
    ...
    public static Person Procreate(Person p1, Person p2)
    {
        ...
        Person baby = new() //se create a new object
        {
        ...
        return baby; //so it's good to return this object
    }
    ...
    #region Interface IComparable - implementations
    //it's better to create it in a separate class (like IComparer)

    public int CompareTo(Person? other)
    //public interface IComparable<in T uses method: int CompareTo(T? other);
    {
        int position;
